			...
			default:
				...
				for (section in noteData) {
					...
					for (songNotes in section.sectionNotes)
					{
						...
						var daNoteOppos:Bool = false;
						var daNoteDirectionOverride:Int = 0;

						// very stupid but I'm lazy
						if (songNotes.length > 2)
							daNoteAlt = songNotes[3];
						if (songNotes.length > 3)
							daNoteOppos = songNotes[4];
						if (songNotes.length > 4)
							daNoteDirectionOverride = songNotes[5];
						/*
						...
						// create the new note
						var swagNote:Note = ForeverAssets.generateArrow(PlayState.assetModifier, daStrumTime, daNoteData, 0, daNoteAlt, daNoteOppos, daNoteDirectionOverride);
						// set note speed
						...
						for (susNote in 0...Math.floor(susLength))
						{
							...
							var sustainNote:Note = ForeverAssets.generateArrow(PlayState.assetModifier,
								daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, 0, daNoteAlt, daNoteOppos, daNoteDirectionOverride, true, oldNote);
